...
| [Run the evaluator from JUnit](junit-adapter.md) | You want the complete configured evaluator verdict as an in-process JUnit Jupiter assertion. |
...
`CycleGovernanceEvaluator.evaluate`. Repository-scope integrations use `RepositoryScopeRepository`,
`RepositoryScopeMatcher`, and `RepositoryScopeEvaluationContext`. Baseline-capable tools can use `CycleDebtBaselinePlanner` to
