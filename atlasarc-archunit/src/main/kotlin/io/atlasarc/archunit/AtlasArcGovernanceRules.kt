...
/** Java-friendly entry point for AtlasArc's repository-backed ArchUnit rule. */
...
class AtlasArcGovernedCyclesRuleBuilder internal constructor() {
    ...
    fun forAnalysisSource(id: String): AtlasArcGovernedCyclesRuleBuilder = apply {
        require(id.isNotBlank()) { "The AtlasArc analysis source ID must not be blank." }
        analysisSourceId = id
    }
    ...
    fun build(): ArchRule {
        require(sourceRoots.isNotEmpty()) {
            "Declare at least one Java/Kotlin source root so the ArchUnit rule can reproduce AtlasArc evidence identities."
        }
        ...
        require(!(hasNamedRoots && hasModulelessRoots)) {
            "Do not mix named and module-less AtlasArc source roots. Use withModuleSourceRoot for every root in a " +
...
private class AtlasArcGovernedCyclesRule(
    ...
    private val description: String = "be free of cycles not accepted by AtlasArc repository governance",
    ...
    override fun evaluate(classes: JavaClasses): EvaluationResult {
        if (classes.toList().isEmpty()) {
            return if (allowEmpty) success() else failure(classes, "AtlasArc cannot evaluate an empty ArchUnit class set.")
        }
        ...
        val loaded = when (val read = CycleGovernanceRepository().read(repositoryStart)) {
            ...
            is GovernanceReadResult.Invalid -> return failure(
                classes,
                "AtlasArc governance is invalid: " + read.issues.joinToString("; ") { it.message },
            )
            is GovernanceReadResult.MissingVcsRoot -> return failure(
                classes,
                "AtlasArc could not find the Git repository that owns .atlasarc/governance/cycles.json.",
            )
            is GovernanceReadResult.IoError -> return failure(
                classes,
                "AtlasArc could not read repository governance: ${read.message}",
            )
        }
        ...
        val result = try {
            ...
            return failure(
                classes,
                "AtlasArc could not evaluate the ArchUnit class set: ${exception.message ?: exception.javaClass.simpleName}",
            )
        }
        ...
        when (result.verdict) {
            ...
            GovernanceEvaluationVerdict.PROBLEMS -> result.problemGroups.forEach { group ->
                events.add(
                    SimpleConditionEvent.violated(
                        group,
                        "AtlasArc found an ungoverned cycle in '${group.analysisSourceId}': " +
            ...
            GovernanceEvaluationVerdict.INVALID -> {
                if (result.issues.isEmpty()) {
                    events.add(SimpleConditionEvent.violated(result, "AtlasArc governance evaluation is invalid."))
                } else {
                    result.issues.forEach { issue ->
                        events.add(SimpleConditionEvent.violated(issue, "AtlasArc governance is invalid: ${issue.message}"))
                    }
                }
            }
        }
